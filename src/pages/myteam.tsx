...
type FlowStep = 'overview' | 'choose-type' | 'choose-existing' | 'create-team' | 'individual-confirm' | 'goalkeeper-confirm';
...
function MyTeam() {
  ...
  const [eventStatuses, setEventStatuses] = useState<{ [battlenightId: string]: EventStatus }>({});
  ...
  const loadData = async () => {
    ...
    try {
      ...
      playerTeams.forEach((t: Team) => {
        if (!allMyTeams.find(lt => lt.id === t.id)) allMyTeams.push(t);
      });
      ...
      await Promise.all(
        events.filter(e => e.status === 'open').map(async (e) => {
          if (e.id) {
            const status = await getUserEventStatus(e.id, currentUser.userId);
  ...
  const showBanner = (text: string, type: 'success' | 'warning' | 'info' = 'success', duration = 5000) => {
    ...
    if (duration > 0) setTimeout(() => setShowConfirmation(false), duration);
  };
  ...
  const handleCancelIndividual = async (battlenightId: string) => {
    ...
    try {
      ...
      showBanner('✅ Du er afmeldt som individuel spiller');
      await loadData();
    } catch (err) {
      console.error(err);
    }
  ...
  const handleCancelGoalkeeper = async (battlenightId: string) => {
    ...
    try {
      ...
      showBanner('✅ Du er afmeldt som målmand');
      await loadData();
    } catch (err) {
      console.error(err);
    }
  ...
  const handleReuseTeam = async (existingTeam: Team) => {
    ...
    try {
      ...
      await removeGoalkeeperSignup(selectedBattlenight.id!, currentUser.userId);

      const formattedDate = new Date(selectedBattlenight.date).toLocaleDateString('da-DK', { weekday: 'long', day: 'numeric', month: 'long' });
      showBanner(`✅ Holdet "${existingTeam.teamName}" er tilmeldt ${formattedDate}!`);
    ...
    const team: Omit<Team, 'id' | 'createdAt'> = {
      ...
      players: [
        ...
        ...selectedPlayers.map((userId: string) => {
          ...
          if (user) return { userId: user.userId, firstName: user.firstName, playerNumber: user.playerNumber, club: user.club, birthYear: user.birthYear, status: 'pending' as const };
          return { userId, firstName: userId, playerNumber: 0, club: '', birthYear: 0, status: 'placeholder' as const, placeholderName: userId };
        }),
      ],
      equipment,
      paid: false,
  ...
  const handleRespondToInvite = async (invite: TeamInvite, accept: boolean) => {
    ...
    try {
      ...
      if (accept) {
        if (isFull) {
          showBanner(`⏳ Holdet "${invite.teamName}" var fuldt - du er sat på venteliste til ${formattedDate}!`, 'warning', 0);
        } else {
  ...
  const handleInvitePlayer = async (team: Team, userId: string) => {
    ...
    try {
      ...
      if (!alreadyInvited) {
        const updatedPlayers = [...team.players, { userId: user.userId, firstName: user.firstName, playerNumber: user.playerNumber, club: user.club, birthYear: user.birthYear, status: 'pending' as const }];
  ...
  const handleRemovePlayer = async (team: Team, userId: string) => {
    try {
      ...
      await createNotification({ toUserId: userId, type: 'general', title: '⚠️ Fjernet fra holdet', message: `Du er blevet fjernet fra holdet "${team.teamName}"` });
  ...
  const handleLeaveTeam = async (team: Team) => {
    ...
    if (isLeader) {
      ...
      if (updatedPlayers.length === 0) {
        ...
        await updateTeam(team.id!, { players: updatedPlayers, leaderId: newLeader.userId, leaderName: newLeader.firstName });
        await createNotification({ toUserId: newLeader.userId, type: 'general', title: '👑 Du er nu holdleder!', message: `${currentUser.firstName} har meldt afbud. Du er nu holdleder for ${team.teamName}` });
      }
    } else {
      const updatedPlayers = team.players.filter((p: TeamPlayer) => p.userId !== currentUser.userId);
      await updateTeam(team.id!, { players: updatedPlayers });
      await createNotification({ toUserId: team.leaderId, type: 'general', title: '⚠️ Spiller har meldt afbud', message: `${currentUser.firstName} har meldt afbud fra holdet ${team.teamName}` });
    }

    showBanner('⚠️ Du er afmeldt holdet.', 'warning');
    await loadData();
  };
    ...
    <div className="page-container">
      <div className="page-header">
        ...
        <h1 className="page-title">
          ...
          {flowStep === 'goalkeeper-confirm' && 'MÅLMAND'}
        </h1>
        <div />
      </div>

      {showConfirmation && (
        <div className={`confirmation-banner ${confirmationType}`} onClick={() => confirmationType !== 'warning' && setShowConfirmation(false)}>
          <p>{confirmationText}</p>
          {confirmationType === 'warning' && (
            <button className="banner-close-btn" onClick={() => setShowConfirmation(false)}>✕ Forstået</button>
          )}
        </div>
      )}
      ...
      <div className="content">
                    ...
                    <div className="invite-info">
                      ...
                      <p className="invite-date">
                        📅 {invite.battlenightDate
                          ? new Date(invite.battlenightDate).toLocaleDateString('da-DK', { weekday: 'long', day: 'numeric', month: 'long' })
                          : invite.battlenightDate}
                      </p>
                      <p className="invite-hurry">⚡ Skynd dig - først til mølle!</p>
                    </div>
                    <div className="invite-actions">
                      <button className="accept-invite-btn" onClick={() => handleRespondToInvite(invite, true)}>✅ Acceptér</button>
                      <button className="decline-invite-btn" onClick={() => handleRespondToInvite(invite, false)}>❌ Afvis</button>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {battlenights.length > 0 && (
                  <div className="signup-section">
                    ...
                    {battlenights.slice(0, 1).map(bn => {
                      const myStatus = eventStatuses[bn.id!];
                      return (
                        <div key={bn.id} className="next-battlenight-card">
                          ...
                          {myStatus?.status === 'waitlist' && (
                            <div className="current-signup-status waitlist">
                      ...
                      return (
                        <div key={team.id} className="team-card">
                          ...
                          {myPlayerStatus === 'accepted' && (
                            <button className="team-chat-btn" onClick={() => navigate(`/messages?teamId=${team.id}`)}>
                          ...
                          <div className="equipment-row">
                            ...
                            {isLeader && (
                              <button className="change-equipment-btn" onClick={() => updateTeam(team.id!, { equipment: team.equipment === 'full' ? 'basic' : 'full' }).then(loadData)}>
                                Skift
                              </button>
                            )}
                          </div>
                          ...
                          <div className="players-section">
                            <h3 className="players-title">✅ Spillere på holdet ({acceptedPlayers.length}/3)</h3>
                            <div className="players-list">
                              {acceptedPlayers.map((player: TeamPlayer, index: number) => (
                                <div key={index} className="player-card">
                                  <div className="player-avatar">#{player.playerNumber || '?'}</div>
                                  <div className="player-info">
                                    <p className="player-name">{player.firstName}{player.userId === team.leaderId && ' 👑'}</p>
                          ...
                          {isLeader && waitlistPlayers.length > 0 && (
                            <div className="players-section">
                              <h3 className="players-title">
                                ...
                                {acceptedPlayers.length < 3 && <span className="waitlist-hint"> - Du kan rykke en spiller op!</span>}
                              </h3>
                              <div className="players-list">
                          ...
                          {isLeader && (
                            <div className="invite-players-section">
                              ...
                              <div className="invite-search">
                                <input type="text" className="search-input-small" placeholder="Søg spiller i appen..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                                {searchQuery && (
                                  <div className="search-results-dropdown">
                                    {filteredUsers.slice(0, 5).map((user: User) => {
                                      const alreadyInTeam = team.players.some((p: TeamPlayer) => p.userId === user.userId && (p.status === 'accepted' || p.status === 'waitlist'));
                                      return (
                                        <div key={user.id} className="search-result-item">
                                          ...
                                          {alreadyInTeam ? (
                                            ...
                                            <button className="invite-small-btn" onClick={() => handleInvitePlayer(team, user.userId)}>Inviter</button>
                                          )}
                                        </div>
                                      );
                                    })}
                              ...
                              <div className="invite-copy-options">
                                <button className="invite-copy-btn" onClick={() => setShowInviteText(showInviteText === `team-${team.id}` ? null : `team-${team.id}`)}>
                                  📲 Inviter via SMS/Snap {showInviteText === `team-${team.id}` ? '▲' : '▼'}
                                </button>
                                <button className="invite-copy-btn opponent" onClick={() => setShowInviteText(showInviteText === `opp-${team.id}` ? null : `opp-${team.id}`)}>
                              ...
                              {showInviteText === `team-${team.id}` && (
                                <div className="invite-text-box">
                                  ...
                                  <textarea className="invite-textarea" readOnly value={generateInviteText('team', team.teamName, battlenights.find(b => b.id === team.battlenightId)?.date)} rows={9} />
                                  <button className="copy-btn" onClick={() => handleCopy(generateInviteText('team', team.teamName, battlenights.find(b => b.id === team.battlenightId)?.date))}>📋 Kopiér tekst</button>
                                </div>
                              )}
                              ...
                              {showInviteText === `opp-${team.id}` && (
                                <div className="invite-text-box opponent">
                                  ...
                                  <textarea className="invite-textarea" readOnly value={generateInviteText('opponent', team.teamName)} rows={8} />
                                  <button className="copy-btn opponent" onClick={() => handleCopy(generateInviteText('opponent', team.teamName))}>📋 Kopiér udfordring</button>
                                </div>
                              )}
                            </div>
                          )}
          ...
          <div>
            <div className="chosen-event-card">
              ...
              <h3 className="chosen-event-date">
                {new Date(selectedBattlenight.date).toLocaleDateString('da-DK', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
              </h3>
            ...
            <div className="type-choice-cards">
              {pastTeams.length > 0 && (
                <button className="type-choice-card highlight" onClick={() => setFlowStep('choose-existing')}>
                  ...
                  <p className="type-choice-desc">Du har {pastTeams.length} hold du kan genbruge. Holdnavn og rangliste bevares!</p>
                </button>
              )}
              ...
              <button className="type-choice-card goalkeeper" onClick={() => setFlowStep('goalkeeper-confirm')}>
                <span className="type-choice-icon">🥅</span>
                <h3 className="type-choice-title">Tilmeld som målmand</h3>
                <p className="type-choice-desc">Tilmeld dig som løsgænger målmand. Du vises på event oversigten.</p>
              </button>
            </div>
          </div>
        )}
        ...
        {flowStep === 'choose-existing' && selectedBattlenight && (
          <div>
            <div className="chosen-event-card">
              ...
              <h3 className="chosen-event-date">
                {new Date(selectedBattlenight.date).toLocaleDateString('da-DK', { weekday: 'long', day: 'numeric', month: 'long' })}
              </h3>
            </div>
                ...
                <div key={team.id} className="existing-team-card">
                  <div className="existing-team-info">
                    ...
                    <p className="existing-team-details">
                      {team.equipment === 'full' ? '🏒 Fuldt' : '🧤 Basis'} · {team.players.filter(p => p.status === 'accepted').length} spillere sidst
                    </p>
                  </div>
                  <button className="use-team-btn" onClick={() => handleReuseTeam(team)}>Brug dette hold</button>
                </div>
              ))}
            </div>

            <button className="action-btn secondary" style={{ marginTop: '15px' }} onClick={() => setFlowStep('choose-type')}>← Tilbage</button>
          </div>
        )}
        ...
        {flowStep === 'create-team' && selectedBattlenight && (
          <div>
            ...
            <div className="form-section">
              ...
              <input type="text" className="form-input" placeholder={`Team ${currentUser.firstName}`} value={teamName} onChange={(e) => setTeamName(e.target.value)} />
            ...
            <div className="form-section">
              ...
              <div className="equipment-toggle">
                <button className={`equipment-btn ${equipment === 'full' ? 'active' : ''}`} onClick={() => setEquipment('full')}>
                  🏒 Fuldt udstyr<span className="equipment-desc">Alle beskyttere, hjelm, handsker, stav</span>
                </button>
                <button className={`equipment-btn ${equipment === 'basic' ? 'active' : ''}`} onClick={() => setEquipment('basic')}>
                  🧤 Basis udstyr<span className="equipment-desc">Kun stav, handsker og hjelm</span>
                </button>
              </div>
            </div>
            ...
            <div className="form-section">
              ...
              <input type="text" className="form-input" placeholder="Søg efter spiller i appen..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                  ...
                  {filteredUsers.slice(0, 5).map((user: User) => (
                    <div key={user.id} className="search-result-item">
                      ...
                      {selectedPlayers.includes(user.userId) ? (
                        <button className="remove-player-btn" onClick={() => setSelectedPlayers(prev => prev.filter(id => id !== user.userId))}>✕ Fjern</button>
                      ) : (
                        <button className="add-player-small-btn" onClick={() => { setSelectedPlayers(prev => [...prev, user.userId]); setSearchQuery(''); }}>+ Tilføj</button>
                      )}
                    </div>
                  ))}
                </div>
              ...
              <div className="invite-new-section">
                ...
                <button className="show-invite-text-btn" onClick={() => setShowInviteText(showInviteText ? null : 'new-team')}>
                  {showInviteText === 'new-team' ? '▲ Skjul invite tekst' : '📲 Vis invite tekst til SMS/Snap ▼'}
                </button>
                {showInviteText === 'new-team' && (
                  <div className="invite-text-box">
                    <p className="invite-text-label">📋 Kopiér og send til din ven:</p>
                    <textarea className="invite-textarea" readOnly value={generateInviteText('team', teamName || `Team ${currentUser.firstName}`, selectedBattlenight.date)} rows={9} />
                    <button className="copy-btn" onClick={() => handleCopy(generateInviteText('team', teamName || `Team ${currentUser.firstName}`, selectedBattlenight.date))}>📋 Kopiér tekst</button>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="create-actions">
              <button className="action-btn primary" onClick={handleCreateTeam}>✅ Opret hold og tilmeld</button>
              <button className="action-btn secondary" onClick={() => setFlowStep('choose-type')}>← Tilbage</button>
            </div>
          </div>
        )}
            ...
            <div className="create-actions">
              <button className="action-btn primary" onClick={async () => {
                try {
                  ...
                  const formattedDate = new Date(selectedBattlenight.date).toLocaleDateString('da-DK', { day: 'numeric', month: 'long' });
        ...
        {/* ============ GOALKEEPER CONFIRM ============ */}
        {flowStep === 'goalkeeper-confirm' && selectedBattlenight && (
          <div>
            <div className="chosen-event-card">
              <p className="chosen-event-label">Du tilmelder dig som målmand:</p>
              <h3 className="chosen-event-date">
                {new Date(selectedBattlenight.date).toLocaleDateString('da-DK', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
              </h3>
              <p className="chosen-event-details">⏰ {selectedBattlenight.time} · 💰 {selectedBattlenight.price} kr</p>
            ...
            <div className="goalkeeper-info-card">
              <h3>🥅 Som målmand:</h3>
              <ul className="individual-info-list">
                <li>✅ Du tilmeldes som løsgænger målmand</li>
                <li>✅ Du vises på event oversigten som tilgængelig målmand</li>
                <li>✅ Hold og spillere kan se at du kommer</li>
                <li>✅ Du betaler samme pris som andre spillere</li>
                <li>⚠️ Du kan ikke være på et hold som målmand</li>
              </ul>
            </div>

            <div className="create-actions">
              <button className="action-btn goalkeeper" onClick={async () => {
                try {
                  const { signupGoalkeeper } = await import('../services/battlenightService');
                  await signupGoalkeeper(selectedBattlenight.id!, currentUser.userId, currentUser.firstName);
                  const formattedDate = new Date(selectedBattlenight.date).toLocaleDateString('da-DK', { day: 'numeric', month: 'long' });
                  showBanner(`🥅 Du er tilmeldt som målmand til ${formattedDate}!`);
                  setFlowStep('overview');
                  await loadData();
                } catch (err: any) {
                  showBanner(err.message || 'Der skete en fejl', 'warning', 3000);
