...
function Dashboard() {
  ...
  const loadData = async () => {
    try {
      ...
      if (openEvents.length > 0) {
        ...
        const [eventTeams, eventIndividuals, eventGoalkeepers, status] = await Promise.all([
          getTeamsForBattlenight(next.id!),
          getIndividualSignups(next.id!),
          getGoalkeeperSignups(next.id!),
          getUserEventStatus(next.id!, currentUser!.userId),
        ]);

        setTeams(eventTeams);
        setIndividuals(eventIndividuals);
        setGoalkeepers(eventGoalkeepers);
  ...
  const acceptedTeams = teams.filter(t => t.players.some(p => p.status === 'accepted'));

  return (
    <div className="dashboard-container">
      <div className="dashboard-header">
        <div className="header-left">
          <img src="/logo.png" alt="3on3 Battlenight" className="header-logo"
        ...
        <div className="header-right">
          ...
          <div className="balance-badge" onClick={() => navigate('/profile')}>💰 {currentUser.balance || 0} kr</div>
        ...
        {isLoading ? (
          ...
          <div className="battlenight-card">
            <div className="battlenight-header-row">
              ...
              {userStatus.status === 'team' && <div className="user-event-status team">✅ På hold</div>}
              {userStatus.status === 'individual' && <div className="user-event-status individual">🏒 Individuel</div>}
              {userStatus.status === 'waitlist' && <div className="user-event-status waitlist">⏳ Venteliste</div>}
              {userStatus.status === 'goalkeeper' && <div className="user-event-status goalkeeper">🥅 Målmand</div>}
            </div>
            ...
            {userStatus.status === 'team' && userStatus.teamName && (
              <div className="user-status-detail team">
                <p>👥 Du er på holdet <strong>{userStatus.teamName}</strong></p>
                <button className="status-action-btn" onClick={() => navigate('/myteam')}>Se dit hold →</button>
              </div>
            )}
            ...
            {userStatus.status === 'goalkeeper' && (
              <div className="user-status-detail goalkeeper">
                <p>🥅 Du er tilmeldt som målmand</p>
                <p className="status-note">Du vises som tilgængelig målmand på event oversigten</p>
              </div>
            )}
            {userStatus.status === 'none' && (
              <div className="join-options">
                <button className="join-btn" onClick={() => navigate('/myteam')}>🏒 Tilmeld dig nu</button>
              </div>
            )}
            ...
            <div className="spots-container">
              ...
              <p className="spots-text">
                ...
                {goalkeepers.length > 0 && ` · ${goalkeepers.length} målmand${goalkeepers.length !== 1 ? 'e' : ''}`}
              </p>
            </div>

            <button className="show-teams-btn" onClick={() => setShowTeamList(!showTeamList)}>
            ...
            {showTeamList && (
              <div className="teams-overview">
                {acceptedTeams.length > 0 && (
                  <div className="teams-overview-section">
                    ...
                    {acceptedTeams.map(team => {
                      ...
                      return (
                        <div key={team.id} className="team-overview-card" onClick={() => navigate(`/messages?teamId=${team.id}`)}>
                          <div className="team-overview-info">
                            <p className="team-overview-name">{team.teamName}</p>
                            <p className="team-overview-players">{acceptedPlayers.map(p => p.firstName).join(', ')}</p>
                            <span className={`team-overview-equipment ${team.equipment}`}>
                              {team.equipment === 'full' ? '🏒 Fuldt' : '🧤 Basis'}
                            </span>
                          </div>
                          <div className="team-overview-actions">
                            <button className="contact-team-btn" onClick={(e) => { e.stopPropagation(); navigate(`/messages?teamId=${team.id}`); }}>💬</button>
                          </div>
                        </div>
                      );
                    })}
                    ...
                    <div className="individuals-list">
                      {(individuals as any[]).map((player: any) => (
                        <div key={player.id} className="individual-overview-card">
                          ...
                          <button className="contact-individual-btn" onClick={() => navigate('/messages')}>💬 Skriv</button>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {goalkeepers.length > 0 && (
                  <div className="teams-overview-section">
                    <h4 className="teams-overview-title">🥅 Målmænd ({goalkeepers.length})</h4>
                    <div className="individuals-list">
                      {(goalkeepers as any[]).map((gk: any) => (
                        <div key={gk.id} className="individual-overview-card goalkeeper-card">
                          <span className="individual-name">🥅 {gk.userName}</span>
                          <button className="contact-individual-btn" onClick={() => navigate('/messages')}>💬 Skriv</button>
                        </div>
                      ))}
                    </div>
                ...
                {acceptedTeams.length === 0 && individuals.length === 0 && goalkeepers.length === 0 && (
      ...
      <div className="section">
        ...
        <div className="stats-grid">
          <div className="stat-card"><span className="stat-number">0</span><span className="stat-label">Deltagelser</span></div>
          <div className="stat-card official"><span className="stat-number">0</span><span className="stat-label">Officielle Sejre</span></div>
          <div className="stat-card official"><span className="stat-number">0</span><span className="stat-label">Officielle Nederlag</span></div>
          <div className="stat-card"><span className="stat-number">-</span><span className="stat-label">Rangliste</span></div>
        </div>
        ...
        <div className="menu-grid">
          <button className="menu-card" onClick={() => navigate('/myteam')}>
            <span className="menu-icon-wrapper">
              ...
              {pendingInvites > 0 && <span className="menu-badge">{pendingInvites}</span>}
            </span>
            <span className="menu-label">Mit Hold</span>
          </button>
          <button className="menu-card" onClick={() => navigate('/calendar')}><span className="menu-icon">📅</span><span className="menu-label">Kalender</span></button>
          <button className="menu-card" onClick={() => navigate('/messages')}><span className="menu-icon">💬</span><span className="menu-label">Beskeder</span></button>
          <button className="menu-card" onClick={() => navigate('/leaderboard')}><span className="menu-icon">🏆</span><span className="menu-label">Rangliste</span></button>
          <button className="menu-card" onClick={() => navigate('/food')}><span className="menu-icon">🍔</span><span className="menu-label">Mad & Drikke</span></button>
          <button className="menu-card" onClick={() => navigate('/rules')}><span className="menu-icon">📋</span><span className="menu-label">Regler</span></button>
          ...
          {(currentUser.role === 'admin' || currentUser.role === 'superadmin') && (
            <button className="menu-card admin" onClick={() => navigate('/admin')}><span className="menu-icon">🛡️</span><span className="menu-label">Admin</span></button>
          )}
          {currentUser.role === 'superadmin' && (
            <button className="menu-card superadmin" onClick={() => navigate('/superadmin')}><span className="menu-icon">⚙️</span><span className="menu-label">Super Admin</span></button>
          )}
        </div>
      </div>
